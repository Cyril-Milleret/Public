...
#' If gamma, w, h or phi are assumed to be spatially heterogeneous, a vector of probability should be provided for gammaVec, wSpatial, hSpatial or phiSpatial.
#' if gamma, w, h or phi are assumed to be spatially homogeneous, a scalar should be provided for w, h or phi.
#' 
#' 
#' @name dcatState2Dead 
#' 
...
#' @param zit Scalar of the initial individual state z_{i,t}
...
#' @param phi Scalar with probability w to transition from z_{i,t} = 2 to z_{i,t+1} = 2.
#' @param hSpatial Vector with probability w_r to transition from z_{i,t} = 2 to z_{i,t+1} = 3 for each cell r of the habitat. The vector should be of the length of the number of habitat windows in \code{\link{habitatGrid}} .
...
#' @param phiSpatial Vector with probability w_r to transition from z_{i,t} = 2 to z_{i,t+1} = 2 for each cell r of the habitat. The vector should be of the length of the number of habitat windows in \code{\link{habitatGrid}} .
...
#' \code{RcatState2Dead} gives a randomly generated individual states conditionnal on the initial state \code{zit}.  
#' 
...
#' sxy <- rbernppAC(n=1, lowerCoords, upperCoords, logIntensities, logSumIntensity, 
#'                  habitatGrid, numGridRows, numGridCols)
#' 
#' ## No spatial mortality 
#' zitPlusOne <- rcatState2Dead( zit = zit
#'                               , gamma = gamma
#'                               , h = h
#'                               , w = w
#'                               , phi = phi
...
dcatState2Dead  <- nimbleFunction(run = function( x = double(0)
  ...
  if(zit == 1){
    ...
    if(gamma == -999){
      sxyID <- habitatGrid[trunc(sxy[2])+1, trunc(sxy[1])+1]
      IndGamma <- gammaVec[sxyID]
...
registerDistributions(list(
  dcatState2Dead = list(
    BUGSdist = "dcatState2Dead(zit, gamma       , gammaVec      , h       , hSpatial      , w       , wSpatial      , phi       , phiSpatial      ,  sxy, habitatGrid)",
    ...
    Rdist = c( "dcatState2Dead(zit, gamma       , gammaVec = sxy, h       , hSpatial = sxy, w       , wSpatial = sxy, phi       , phiSpatial = sxy,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma       , gammaVec = sxy, h       , hSpatial = sxy, w = -999, wSpatial      , phi = -999, phiSpatial      ,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma       , gammaVec = sxy, h = -999, hSpatial      , w = -999, wSpatial      , phi = -999, phiSpatial      ,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma       , gammaVec = sxy, h = -999, hSpatial      , w       , wSpatial = sxy, phi = -999, phiSpatial      ,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma = -999, gammaVec      , h       , hSpatial = sxy, w       , wSpatial = sxy, phi       , phiSpatial = sxy,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma = -999, gammaVec      , h       , hSpatial = sxy, w = -999, wSpatial      , phi = -999, phiSpatial      ,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma = -999, gammaVec      , h = -999, hSpatial      , w = -999, wSpatial      , phi = -999, phiSpatial      ,  sxy, habitatGrid)",
               "dcatState2Dead(zit, gamma = -999, gammaVec      , h = -999, hSpatial      , w       , wSpatial = sxy, phi = -999, phiSpatial      ,  sxy, habitatGrid)"
               ),
               
               
    types = c( "value = double(0)", "zit = double(0)","gamma = double(0)", "gammaVec = double(1)", "h = double(0)", "hSpatial = double(1)" , "w = double(0)", "wSpatial = double(1)", "phi = double(0)", "phiSpatial = double(1)", "sxy = double(1)", "habitatGrid = double(2)"
    ),
    discrete = TRUE,
    mixedSizes = TRUE,
    pqAvail = FALSE
