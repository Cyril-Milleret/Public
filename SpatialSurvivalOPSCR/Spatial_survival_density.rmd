...
```{r , warning = FALSE, message = FALSE}
r <- raster(nrows=5, ncols=5, xmn=10, xmx=50, ymn=10, ymx=50)
r[] <- 1
## Create habitat grid
...
## Create trap grid
coordsObsCenter <- cbind(rep(seq(12.5, 47.5, by = 1), 36),
                         sort(rep(seq(47.5, 12.5, by = -1), 36)))
...
### check surv/mortality prob relationship 
dens <- log(c(1:10))
...
Here, we build the NIMBLE model again using the simulated 'y' and 'y.dead' as data. For simplicity, we used the simulated 'z' as initial values. Then we can fit the SCR model with the simulated 'y' 'y.dead' data set.  
...
MCMCconf <- configureMCMC(model = model,
                          monitors = c("p0","sigma","betaPhi","phi0", "N",
                                         "gamma","tau"),
