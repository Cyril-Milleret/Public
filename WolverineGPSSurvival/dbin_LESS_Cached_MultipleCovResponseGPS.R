...
#### 1.Density function ####
dbin_LESS_Cached_MultipleCovResponseGPS <- nimbleFunction(run = function( x = double(1),
                                                          sxy = double(1),
                                                          sigma = double(0),
   ...
   for(j in 1:nDetectors){
      if(index1[count] == j){ # IF J IS EQUAL TO THE RELEVANT DETECTOR 
         ...
         pZero <- ilogit(logit(p0State[detCountries[j]]) +
                            ...
                            BetaResponse*detResponse + BetaGPS*detGPS)
...
registerDistributions(list(
  dbin_LESS_Cached_MultipleCovResponseGPS = list(
      BUGSdist = "dbin_LESS_Cached_MultipleCovResponseGPS(sxy, sigma, nbDetections, yDets, 
      ...
      types = c( "value = double(1)","sxy = double(1)", "sigma = double(0)",
                 ...
                 "BetaGPS = double(0)", "detGPS = double(0)"),
      pqAvail = FALSE)))
